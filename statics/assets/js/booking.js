...
function kamaDatepicker(elementID, opt) {
  ...
  const game_name = document.getElementById('this_page').innerHTML;
  const data = {
    'game_name':game_name
  }
  $.ajaxSetup({
  ...
  $.ajax({
      ...
      success : function (res, status) {
          console.log('suc');
          let past_days = res.past_days;
          let closed_days = res.closed_days;

          past_days.forEach(element => {
            var past_day = document.getElementsByClassName('day-'+element)[0]
            past_day.classList.add('past_day');
            past_day.setAttribute('disabled', '');
            past_day.removeAttribute('onclick', 'function()');
          });

          closed_days.forEach(element => {
            var closed_day = document.getElementsByClassName('day-'+element)[0]
            closed_day.classList.add('closed_day');
            closed_day.setAttribute('onclick', 'showform('+ '"' + element + '"' +')');
  ...
  inputElement.after(
    "<div id='bd-main-" +
      elementID +
      "' class='bd-main ' style=' direction: rtl;'></div>"
  );
...
//   monthDropdown.val(selectedMonth);
  ...
  neededDays = false
  var neededNextMDay = 10 - (numberOfDays - todaysDay);
  if (neededNextMDay <= 10) {
    neededDays = true
  }
  ...
  var drawDays = function (numberOfDays, dayOfWeekJ) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (i < 7 && dayIndex <= numberOfDays) {
          var tempTD =
            ...
            dayIndex +
            '" type="button" onclick="showform('+dayIndex+')">' +
            (options.forceFarsiDigits ? FA_NUMS[dayIndex] : dayIndex) +
            '</button>' +
            '</td>';
          var latestDayIndex = dayIndex;
          var latestWeekday = i
    ...
    if (neededDays) {
      ...
      for (let index = 1; index < neededNextMDay+1; index++) {
        latestWeekday ++
        if (latestWeekday < 7) {

          var tempTD =
            '<td>' +
            '<button class="day day-0' +
            index +
            '" type="button" onclick="showform('+index+')">' +
            (options.forceFarsiDigits ? FA_NUMS[+index] : +index) +
            '</button>' +
            '</td>';
          $('#bd-table-days-' + elementID + ' .tr-' + rowIndex).append(tempTD);
        
        }else if (latestWeekday == 7) {
...
//   });
  ...
  const MONTH_NAMES_LIST = Object.entries(MONTH_NAMES)
  MONTH_NAMES_LIST.forEach(([key, value]) => {
    if (key == todaysMonth) {
  ...
  titleDiv.append(
    "<div class='form-control month text-center'>"+todaysMonth+"</div>"
    );
...
function showform(day) {
  selected_day = document.getElementsByClassName('day-'+day)[0];
  if (selected_day.classList.contains('closed_day')) {
    document.getElementById('BTNS_booking').classList.remove('d-none')
    document.getElementById('select_booking').classList.add('d-none');
    document.getElementById('close_time').classList.add('d-none');
    document.getElementById('close_time').classList.remove('d-flex');
    document.getElementById('open_time').classList.add('d-none');
    document.getElementById('open_time').classList.remove('d-flex');
    document.getElementById('sold_time').classList.add('d-none');
    document.getElementById('sold_time').classList.remove('d-flex');
    document.getElementById('buy_time').classList.add('d-none');
    document.getElementById('buy_time').classList.remove('d-flex');
  } else {
    document.getElementById('BTNS_booking').classList.remove('d-none')
    document.getElementById('close_time').classList.remove('d-none');
    document.getElementById('close_time').classList.add('d-flex');
    document.getElementById('open_time').classList.remove('d-none');
    document.getElementById('open_time').classList.add('d-flex');
    document.getElementById('sold_time').classList.remove('d-none');
    document.getElementById('sold_time').classList.add('d-flex');
    document.getElementById('buy_time').classList.remove('d-none');
    document.getElementById('buy_time').classList.add('d-flex');
    var name = document.getElementById('this_page').innerHTML;
    const data = {
      'day': day,
      'name': name
    }
    ...
    $.ajax({
        ...
        success : function (res, status) {
            console.log('suc');
            if (res.status == 'time'){
              var game_time = JSON.parse(res.game_time_json);
              var time_closed_id = res.time_closed_id;
              var time_sold_id = res.time_sold_id;
              var game_time_select = document.getElementById('game_time');
              game_time_select.innerHTML = '';
              var gtused_id = []
              
              if (time_sold_id.length != 0) {
                console.log('a2')
                game_time.forEach(gametime => {
                  time_sold_id.forEach(soldID => {
                    if (gametime.pk == soldID) {
                      game_time_select.innerHTML += `<option class="game_time_option" style="color:red" value="${gametime.pk}" id="select_${gametime.pk}">${gametime.fields.time_from} تا ${gametime.fields.time_to}  فروخته</option>`
                      gtused_id.push(gametime.pk)
                    }
                  })
                });
              }
              if (time_closed_id.length != 0) {
                console.log('a3')
                game_time.forEach(gametime => {
                  time_closed_id.forEach(closedID => {
                    if (gametime.pk == closedID) {
                      game_time_select.innerHTML += `<option class="game_time_option" style="color:red" value="${gametime.pk}" id="select_${gametime.pk}">${gametime.fields.time_from} تا ${gametime.fields.time_to}  بسته</option>`
                      gtused_id.push(gametime.pk)
                    }
                  })
                });
              ...
              var gtall_id = []
              game_time.forEach(gt => {
                gtall_id.push(gt.pk)                
              });
              // حذف مقدار هایی از لیست ایدی گیم تایم ها که در لیست ایدی مقدار های استفاده شده وجود دارند
              let gtunused_id = gtall_id.filter(function(item) {
                return !gtused_id.includes(item);
              });

              game_time.forEach(gt => {
                gtunused_id.forEach(unused_id => {
                  if (gt.pk == unused_id) {
                    game_time_select.innerHTML += `<option class="game_time_option" value="${gt.pk}" id="select_${gt.pk}">${gt.fields.time_from} تا ${gt.fields.time_to}</option>`
                  }
                });
              });


              console.log(gtused_id)
              console.log(gtunused_id)
             



              document.getElementById('select_booking').classList.remove('d-none')
              document.getElementById('BTNS_booking').classList.remove('d-none')
...
document.getElementsByName("close_time_BTN")[0].addEventListener('click', function () {
  var time = document.getElementsByName('game_time')[0].value;
  var name = document.getElementById('this_page').innerHTML;
  var day = document.getElementsByClassName('bd-selected-day')[0].innerHTML;
  const data = {
    'time' : time,
    'name' : name,
    'day' : day
  ...
  $.ajax({
      ...
      success : function (res, status) {
          ...
          if (res.status == 200) {
            console.log(res.game_time_id);
            document.getElementById('danger_alert').classList.add('d-none');
            document.getElementById('success_alert').classList.remove('d-none');
            document.getElementById('msg-success').innerHTML = res.msg;
            document.getElementById('select_'+res.game_time_id).innerHTML += '                   بسته';
            document.getElementById('select_'+res.game_time_id).style.color = '#e7515a';
...
document.getElementsByName("open_time_BTN")[0].addEventListener('click', function () {
  var time = document.getElementsByName('game_time')[0].value;
  var name = document.getElementById('this_page').innerHTML;
  var day = document.getElementsByClassName('bd-selected-day')[0].innerHTML;
  const data = {
  ...
  $.ajax({
      ...
      success : function (res, status) {
          ...
          if (res.status == 200) {
            ...
            document.getElementById('success_alert').classList.remove('d-none');
            document.getElementById('msg-success').innerHTML = res.msg;
            var selected = document.getElementById('select_'+res.game_time_id)
            selected.innerHTML = res.game_time_val;
...
document.getElementsByName("sold_time_BTN")[0].addEventListener('click', function () {
  var time = document.getElementsByName('game_time')[0].value;
  var name = document.getElementById('this_page').innerHTML;
  var day = document.getElementsByClassName('bd-selected-day')[0].innerHTML;
  const data = {
  ...
  $.ajax({
      ...
      success : function (res, status) {
          ...
          if (res.status == 200) {
            ...
            document.getElementById('success_alert').classList.remove('d-none');
            document.getElementById('msg-success').innerHTML = res.msg;
            document.getElementById('select_'+res.game_time_id).innerHTML += '                   فروخته';
            document.getElementById('select_'+res.game_time_id).style.color = '#e7515a';
          }else if (res.status == 500) {
            document.getElementById('success_alert').classList.add('d-none');
            document.getElementById('danger_alert').classList.remove('d-none');
...
document.getElementsByName("buy_time_BTN")[0].addEventListener('click', function () {
  var time = document.getElementsByName('game_time')[0].value;
  var name = document.getElementById('this_page').innerHTML;
  var day = document.getElementsByClassName('bd-selected-day')[0].innerHTML;
  const data = {
...
document.getElementsByName("close_day_BTN")[0].addEventListener('click', function () {
  var name = document.getElementById('this_page').innerHTML;
  var day = document.getElementsByClassName('bd-selected-day')[0].innerHTML;
  console.log(day)
  const data = {
  ...
  $.ajax({
      ...
      success : function (res, status) {
          ...
          if (res.status == 200) {
            console.log(res.msg)
            console.log(typeof(res.day))
            document.getElementById('danger_alert').classList.add('d-none');
            document.getElementById('success_alert').classList.remove('d-none');
            document.getElementById('msg-success').innerHTML = res.msg;
            window.location.reload();

            // document.getElementsByClassName('day day-'+res.day)[0].classList.add('closed_day');
            // document.getElementById('select_booking').classList.add('d-none');
            // document.getElementById('close_time').classList.add('d-none');
            // document.getElementById('close_time').classList.remove('d-flex');
            // document.getElementById('open_time').classList.add('d-none');
...
document.getElementsByName("open_day_BTN")[0].addEventListener('click', function () {
  var name = document.getElementById('this_page').innerHTML;
  var day = document.getElementsByClassName('bd-selected-day')[0].innerHTML;
  console.log(day)
  const data = {
